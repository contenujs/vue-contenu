...
class EventHandler {
  constructor() {
    ...
    _defineProperty(this, "newFieldRequested", false);
        ...
        case "cssRules":
          if (event.data.value) {
            ...
            for (let key in event.data.value) {
              IFrameInitializer.contenuIframe.style[key] = event.data.value[key];
            }
          }
  ...
  requestNewFields(objPath = null) {
    if (objPath != null) this.eventQueue.push({
      type: "newField",
      data: objPath
    });
...
class IFrameInitializer {
  constructor(serverUrl, key) {
    ...
    IFrameInitializer.contenuIframe.setAttribute("style", ["position:fixed", "border:0", "width:0", "height:0", "overflow: hidden"].join(";"));
    IFrameInitializer.contenuIframe.setAttribute("src", serverUrl + "?key=" + key);
  }
  ...
  remove() {
    IFrameInitializer.contenuIframe.parentNode.removeChild(IFrameInitializer.contenuIframe);
  }

}

_defineProperty(IFrameInitializer, "contenuIframe", null);
...
_defineProperty(Contenu, "data", {});

_defineProperty(Contenu, "props", {});
